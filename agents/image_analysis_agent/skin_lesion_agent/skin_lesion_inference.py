...
class SkinLesionSegmentation:
    ...
    def _overlay_mask(self, img, mask, output_path):
        """将分割掩膜叠加在原始图像上"""
        try:
            mask_stacked = np.stack((mask,) * 3, axis=-1)    # 将掩膜扩展为与原始图像相同
            fig, ax = plt.subplots(figsize=(10, 10))    # 创建一个figure
            ax.axis("off")
            ax.imshow(img)
            ax.imshow(mask_stacked, alpha=0.4)    # 叠加掩膜
    ...
    def predict(self, image_path, output_path):
        ...
        try:
            ...
            img_tensor = torch.Tensor(img_resized).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)  # Convert to tensor

            with torch.no_grad():
                generated_mask = self.model(img_tensor).squeeze().cpu().numpy()    # Generate mask

            # Resize mask to match original image dimensions
            generated_mask_resized = cv2.resize(generated_mask, (img.shape[1], img.shape[0]))  # 将生成的掩码调整为与原始图像相同的尺寸
            return self._overlay_mask(img, generated_mask_resized, output_path)   # 返回叠加结果
